...
var (
	...
	mapByID   map[int]*Currency
	mapByCode map[string]*Currency
)

func init() {
	n := len(currencies)
	mapByID = make(map[int]*Currency, n)
	mapByCode = make(map[string]*Currency, n)
...
// Currency is a representation of a currency, as defined in the ISO 4217 standard
...
// Equals tests if two currencies are equal
func (c Currency) Equals(currency Currency) bool {
	...
	if c.Decimals != currency.Decimals {
		return false
	}
...
// GetByCode returns a currency object for a given ISO 4217 code
func GetByCode(code string) (Currency, error) {
	if cur := mapByCode[code]; cur != nil {
		return *cur, nil
	}
	return Currency{}, ErrCurrencyNotFound
}

// GetByID returns a currency object for a given ISO 4217 numeric ID
func GetByID(ID int) (Currency, error) {
	if cur := mapByID[ID]; cur != nil {
		return *cur, nil
	}
	return Currency{}, ErrCurrencyNotFound
}

// GetList returns a list of currencies matching the given ISO codes, o all if no codes are given.
func GetList(codes ...string) []Currency {
	if len(codes) != 0 {
		var list []Currency
		for _, code := range codes {
			cur, err := GetByCode(code)
			if err != nil {
				continue
			}
	...
	list := make([]Currency, len(currencies))
...
var currencies = []*Currency{
	...
	{"CNH", 156, 2, "Chinese yuan when traded in Hong Kong"},
	{"CNY", 156, 2, "Chinese yuan"},
	{"COP", 170, 2, "Colombian peso"},
	{"COU", 970, 4, "Unidad de Valor Real (UVR) (funds code)[7]"},
	...
	{"XFU", 0, 0, "UIC franc (special settlement currency)"},
